...
\IEEEauthorblockA{\textit{Department of Data Science and Cyber Security}\\
...
Lewis et al.~\cite{lewis2020retrieval} attached a dense retriever to a
Transformer so the model could consult external documents at inference
time, reporting large gains on tasks that demand factual knowledge. Izacard
and Grave~\cite{izacard2021leveraging} then read several retrieved passages
jointly inside the decoder instead of scoring them one by one. Neither
...
Edge et al.~\cite{edge2024local} first let an LLM convert a corpus into an
entity graph and pre-summarise each detected community, trading retrieval
precision for global coverage. Xiong et al.~\cite{xiong2021answering}
instead decompose a complex question into a chain of retrieval steps,
fetching one more passage per hop. Both rely on unstructured text extraction; neither grounds
...
For embeddings we adopt MedCPT~\cite{jin2023medcpt}, a pair of
contrastively trained encoders whose PubMed query--article training log
gives them a clinical vocabulary that general-purpose encoders lack; both
encoders output 768-dimensional vectors. We use the
...
The vector index runs on FAISS~\cite{johnson2021billion}, whose
inverted-file and product-quantisation schemes compress billion-scale
corpora into a few hundred megabytes while keeping search in the
millisecond range. We use an IVFpq index occupying 240~MB on CPU, achieving
...
The Step-11 API suite recorded 2.76~GB RAM / 4{,}784~MB VRAM over
61{,}349~ms (cold 13{,}675~ms; warm 3{,}667~ms); the Step-13 feature-layer
suite recorded 3.93~GB / 4{,}776~MB over 176.68~s---all within budget
(Fig.~\ref{fig:resource}).
...
\textbf{Reproducibility.} Every value is extracted verbatim from the
evaluation artifacts accompanying the submission; none were recomputed, and
all will be published with the camera-ready.
...
\emph{stable}, and computes a rate of change per month. In the Step-13 evaluation:
...
access (1.10~ms, 117~bytes). All 7/7 Step-11 API security criteria passed.
...
For a tool operating on a 50-question USMLE sample without full medical
knowledge, refusing is the correct response. The wrong-assertion rate of 12\%
...
to $N=500$ and fine-tuned models is a natural extension. Median latency (15{,}386~ms) exceeds comfortable interactive thresholds;
batched verification is future work.

\noindent\textbf{iv) Broader impact.}\\*
MedGraphRAG is a patient-safety tool, not a diagnosis engine: every
response carries a mandatory disclaimer and never claims diagnostic
authority, and per-user stores are encrypted at rest.

\subsection{Ethics Statement}
No real patient data were used: the feature layer was validated on
synthetic fixture reports and the benchmark (MedQA-US) is a public
examination dataset. MedGraphRAG is an information tool, not a diagnostic
device; every non-refused response carries a mandatory disclaimer and the
system never claims clinical authority. Per-user stores are encrypted with
AES-256-GCM and isolated at the API layer, and guest data are purged on
logout. No human subjects were involved, so no ethics-board approval was
required.
