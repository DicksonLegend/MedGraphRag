...
% ─── Abstract ────────────────────────────────────────────────────────────────
\begin{abstract}
MedGraphRAG is a self-verifying hybrid graph-RAG system for medical question
answering on consumer hardware, combining dense vector retrieval with a
biomedical knowledge graph and a claim-level verification agent.
Prior medical RAG systems report accuracy on answered questions but
rarely handle insufficient evidence; we treat confidently wrong assertions as
the primary failure mode and structure the system to refuse rather than guess.
...
from 44\% (baseline) to 12\% (full system), trading raw answered-rate for
...
al.~\cite{chen2024benchmarking} showed that accuracy on answered items
substantially overstates safety when the model cannot decline to answer. Our
...
\begin{table}[ht]
\caption{Feature comparison across retrieval systems. ✓~=~fully supported;
...
\toprule
\textbf{Capability} &
\rotatebox{70}{\textbf{RAG~\cite{lewis2020retrieval}}} &
\rotatebox{70}{\textbf{FiD~\cite{izacard2021leveraging}}} &
\rotatebox{70}{\textbf{MH-DR~\cite{xiong2021answering}}} &
\rotatebox{70}{\textbf{GraphRAG~\cite{edge2024local}}} &
\rotatebox{70}{\textbf{Self-RAG~\cite{asai2024selfrag}}} &
...
\textbf{K\`{u}zu property graph}~\cite{su2023kuzu}. The graph contains
...
\caption{Per-configuration breakdown: correct / wrong-asserted / refused
  query counts ($N=50$). Source: evaluation artifact
  step12\_part3\_paper\_tables.json.}
...
Fig.~\ref{fig:disposition} shows the per-configuration count breakdown
(correct / wrong-asserted / refused) across all 50 queries.
...
\begin{itemize}
  \item \textbf{P01} (\emph{warfarin INR monitoring AF}):\\
        Disease(Atrial Fibrillation) $\rightarrow$
        Document(PubMed/AF/PMID\_36356656) $\rightarrow$ Chunk(...abs)
  \item \textbf{P02} (\emph{ACE inhibitor hypertension first line}):\\
        Document(NICE/hypertension-...) $\rightarrow$ Chunk(...c1)
  \item \textbf{P03} (\emph{critical haemoglobin transfusion threshold}):\\
        Document(Lab\_rev\_data/Consolidated\_Lab\_Critical\_Values\_Dataset\_CLEANED)
        $\rightarrow$ Chunk(...row3)
\end{itemize}
...
4{,}776~MB over 176.68~s execution time, all within budget.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/resource_chart.pdf}
\caption{Peak RAM and VRAM across evaluation phases (Steps 11, 12, 13) vs.\
  hardware budgets. Source: evaluation artifacts step11\_api\_report.json,
  step12\_part3\_paper\_tables.json, step13\_features\_report.md.}
\label{fig:resource}
\end{figure}

Fig.~\ref{fig:resource} plots measured RAM and VRAM across the three evaluation
phases against the hardware budgets. All phases remain within budget.
...
scores 0.1777--0.1783, 10 distinct documents each).
...
(C1) versus 44\% (C4 baseline) shows that the verification layer is the
...
compute access and the open-source communities behind FAISS, K\`{u}zu,
LangChain, and Hugging Face for making this research reproducible on consumer
hardware.
