...
\rotatebox{60}{\textbf{FiD~\cite{izacard2021leveraging}}} &
...
\midrule
Hybrid vector+graph    & $\times$ & $\times$ & $\times$ & $\checkmark$ & $\times$ & $\checkmark$ \\
Claim-level verify     & $\times$ & $\times$ & $\times$ & $\times$ & $\checkmark$ & $\checkmark$ \\
Structured refusal     & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ & $\checkmark$ \\
Graph provenance       & $\times$ & $\times$ & $\times$ & $\checkmark$ & $\times$ & $\checkmark$ \\
Contamination guard    & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ & $\checkmark$ \\
Encrypted data store   & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ & $\checkmark$ \\
Longitudinal analytics & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ & $\checkmark$ \\
Consumer-GPU deploy    & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ & $\checkmark$ \\
\bottomrule
\end{tabular}
\end{table}
