#!/usr/bin/env python3
"""
MedGraphRAG Backend — Step 8.2 Verification Agent Evaluation
==============================================================
Evaluates the Step 8.2 optimized Verification Agent across 10 diagnostic probes.

Verifies Acceptance Criteria:
  1. ≥ 8/10 probes with total latency < 12,000 ms; median < 12,000 ms.
  2. verification_ms ≤ 5,000 ms on every non-refusal probe.
  3. Refusals still refusal_valid @ faithfulness 1.0.
  4. Blocklisted citations = 0, PMC twin pairs = 0.
  5. Citations + disclaimer = 100%.
  6. VRAM ≤ 5500 MB, RAM < 12 GB.
  7. latency_total == breakdown sum on every probe.
  8. Saves evaluations/step8_2_report.json.
"""
...
logger = logging.getLogger("step8_2_eval")
...
# Baseline latencies from step 8 before patch
OLD_LATENCIES_MS = {
    "P01": 21870.0,
    "P02": 21400.0,
    "P03": 20950.0,
...
def run_step8_2_eval() -> None:
    print("=" * 85)
    print("  MedGraphRAG — Step 8.2 Verification Agent & Latency Evaluation")
    print("=" * 85)
    print(f"  Target Latency Budget : < {settings.query_latency_budget_ms} ms")
    print(f"  Verification Tokens    : {settings.verification_max_tokens}")
    print(f"  Generation Tokens      : {settings.llm_max_tokens}")
    ...
    for probe in PROBE_QUERIES:
        ...
        # ── Check Latency Sum Equality ───────────────────────────────────────
        r_ms = round(res.latency_breakdown.get("retrieval_ms", 0.0), 2)
        c_ms = round(res.latency_breakdown.get("context_ms", 0.0), 2)
        l_ms = round(res.latency_breakdown.get("llm_ms", 0.0), 2)
        v_ms = round(res.latency_breakdown.get("verification_ms", 0.0), 2)
        expected_total = round(r_ms + c_ms + l_ms + v_ms, 2)
        reported_total = round(res.latency_breakdown.get("total_ms", q_latency), 2)
        latency_sum_match = abs(reported_total - expected_total) <= 1.0
        ...
        pass_check = (
            ...
            and hygiene_ok
            and verif_ok
            and refusal_valid_ok
            and latency_sum_match
        )
        ...
        print(f"       Retries executed: {res.retry_count} | Fallback used: {res.fallback_used} | Trajectory: {res.retry_confidence_trajectory}")
        print(f"       Latency: total={q_latency:.1f} ms (Retrieval: {r_ms:.1f} | Context: {c_ms:.1f} | LLM: {l_ms:.1f} | Verifier: {v_ms:.1f} ms)")
        ...
        probe_results.append({
            ...
            "latency_verification_ms": v_ms,
            "latency_breakdown": {
                "retrieval_ms": r_ms,
                "context_ms": c_ms,
                "llm_ms": l_ms,
                "verification_ms": v_ms,
                "total_ms": reported_total,
            },
            "latency_sum_match": latency_sum_match,
            "ram_gb": res.ram_gb,
            "vram_mb": res.vram_mb,
    ...
    report_path = _PROJECT_ROOT / "evaluations" / "step8_2_report.json"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report = {
        "step": 8.2,
        "description": "Verification Agent Latency & Faithfulness Patch Report",
    ...
    print(f"📄 Saved evaluations/step8_2_report.json\n")

    # ── Print Old vs New Latency Comparison Table (SCRIPT-GENERATED FROM JSON) ─
    print("=" * 85)
    print("  OLD vs NEW LATENCY COMPARISON TABLE (Generated from evaluations/step8_2_report.json)")
    print("=" * 85)
